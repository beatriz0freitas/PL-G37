"""Tabela de símbolos usada pela análise semântica."""
...
@dataclass
class Symbol:
    """Informação semântica associada a um identificador."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.kind not in VALID_KINDS:
            raise ValueError(f"Tipo de símbolo inválido: {self.kind}")


class SymbolTable:
    """Tabela simples de símbolos ao nível do programa."""

    def __init__(self):
        self._table: dict[str, Symbol] = {}

    def declare(self, symbol: Symbol, filename: str = "<stdin>") -> Symbol:
        """Regista um símbolo. Rejeita declarações duplicadas."""
